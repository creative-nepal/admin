...
description: >
  Mirror a design-system change into creative-nepal-web, which keeps a byte-identical copy of
...
stay byte-identical with creative-nepal-web:
...
CI cannot catch drift — the two repos are never checked out together. `../web` is the peer
checkout under `creativenepal-platform/`.

## Workflow

```sh
./scripts/sync-ui.sh diff    # drift against ../web, exit 1 if any
